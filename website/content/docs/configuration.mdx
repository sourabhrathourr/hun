...
This value is authoritative for every launch. **hun** rejects the launch if the configured TCP port is already occupied, and a per-port lease prevents concurrent hun services or instances from selecting the same port while an application is still starting. Runtime detection verifies listening sockets against the launched process group before updating the live port shown in the UI, and it never rewrites or overrides the configured port on a later start. If a service binds a different verified port, **hun** stops it and reports the mismatch instead of silently accepting the fallback.
...
**hun** will inject the configured `port` (including an intentional Multitask offset) into this variable. The configured value takes priority over inherited environment variables and values under the service's `env` section.
