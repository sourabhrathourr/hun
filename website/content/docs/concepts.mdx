...
3.  **Per-service port fallback**: Each service keeps its configured port when it is free. If two services both need port 3000, **hun** shifts only the colliding service to 3001 (or the next available offset).
